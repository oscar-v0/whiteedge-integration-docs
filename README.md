...
## Deposit And Withdrawal QR Code Format

Before a customer can start a deposit or withdrawal, the customer needs to be identified. This is achieved by the customer scanning a QR code that contains a unique verification code that is generated by you. This code is used to inform your API of who is attempting a transaction and should allow you to uniquely identify the customer. The contents of the QR code must follow the format below:\
`whiteedge:[deposit|withdrawal]?code={verificationCode}`

Example:\
`whiteedge:deposit?code=OTBlZWI0NTYtNW`\
`whiteedge:withdrawal?code=FiZTMtOGNmZjEw`
...
Once a customer has scanned a deposit/withdrawal QR code, a transaction needs to be initiated. The verification code extracted from the QR code is sent to your API, along with some other transaction details. Your API should verify the code and return a 2xx response to allow the customer to proceed with the transaction.
...
```ts
{
  ...
  // Verification code extracted from the scanned QR code
...
```ts
{
  ...
  // E.g. [{currency: 'GBP', value: 20}, {currency: 'USD', value: 100}]
  receivables: {
    currency: string,
    value: number,
  }[]
